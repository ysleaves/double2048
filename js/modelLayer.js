...
function init1(cxt)
{
    //游戏背景
    context.beginPath();
    drawRoundRect(cxt, margin_left+600, margin_top, game_width, game_width, 5);
    cxt.fillStyle = color_bg;
    cxt.fill();
    ...
    //初始化16个小格子
    updateBoardView1(cxt);
}

function drawBox1(cxt, i, j, nums){

    var x = (i+1)* margin + margin_left+600 + i * box_width;
    var y = (j+1)* margin + margin_top + j * box_width;
...
//刷新View
function updateBoardView1(cxt) {
    for (var i = 0;i < 4; i++) {
        for (var j = 0;j < 4; j++) {
            drawBox1(cxt,i, j, nums[i][j]);
        }
    }
}